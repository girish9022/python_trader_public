...
packages = [
    ...
    ("pyttsx3", "pyttsx3"),
    ("scipy", "scipy"),   # 👈 NEW
    ("NorenRestApiOAuth", "NorenRestApiOAuth"),   # 👈 Correct PyPI package name
...
# Import API (use same library as working example)
from NorenRestApiPy.NorenApi import NorenApi
print("✅ Using NorenRestApiPy (same as working OAuth example)")
...
def Shoonya_login():
    """
    Shoonya Login with OAuth Support (After April 1st, 2025)
    
    Credential Sheet Layout (User_Credential):
    - B2: User ID (e.g., FA145289)
    - B4: Login Method ("New_Session" for OAuth or other for Token-based)
    - B12: Secret_Code (From Shoonya API Dashboard)
    - B13: Auth Code (From OAuth redirect URL)
    - B10: Telegram Bot Token (Optional)
    - B11: Telegram Chat ID (Optional)
    """
    global api
    ...
    try:
        ...
        try:
            class ShoonyaApiPy(NorenApi):
                def __init__(self):
                    NorenApi.__init__(self, host='https://api.shoonya.com/NorenWClientAPI/', websocket='wss://api.shoonya.com/NorenWS/', eodhost='https://api.shoonya.com/chartApi/getdata/')
             
            api = ShoonyaApiPy()
        except Exception as e:
            class ShoonyaApiPy(NorenApi):
                def __init__(self):
                    NorenApi.__init__(self, host='https://api.shoonya.com/NorenWClientAPI/', websocket='wss://api.shoonya.com/NorenWS/')
        ...
        if (LoginMethod == "New_Session"):
            # New OAuth-based login (After April 1st, 2025)
            try:
                auth_code = str(Credential_sheet.range('B13').value)
                Secret_Code = str(Credential_sheet.range('B12').value)
                
                index = auth_code.find(".")
                if index != -1:
                    auth_code = auth_code[:len(auth_code)-2]
                
                index = Secret_Code.find(".")
                if index != -1:
                    Secret_Code = Secret_Code[:len(Secret_Code)-2]
                
                client_id = f'{userid}_U'
                
                print(f"OAuth Login: userid={userid}, client_id={client_id}")
                print(f"Auth Code: {auth_code[:20]}..." if len(auth_code) > 20 else f"Auth Code: {auth_code}")
                print(f"Secret Code: {Secret_Code[:20]}..." if len(Secret_Code) > 20 else f"Secret Code: {Secret_Code}")
                
                # Validate parameters
                if not auth_code or auth_code == "None" or auth_code.lower() == "none":
                    oauth_guide = f"""
╔════════════════════════════════════════════════════════════════════════════╗
║                    🔐 OAUTH AUTH CODE MISSING (B13)                        ║
╠════════════════════════════════════════════════════════════════════════════╣
║ 1. Open this URL in your browser (replace FA145289 with your client_id):   ║
║    https://trade.shoonya.com/OAuthlogin/investor-entry-level/login?api_key={client_id}&route_to={userid}    ║
║                                                                            ║
║ 2. Login with your Shoonya credentials                                     ║
║                                                                            ║
║ 3. You'll be redirected to a URL with 'code=XXXXXXX-XXXX-...' parameter    ║
║                                                                            ║
║ 4. Copy that code and paste it into cell B13 of User_Credential sheet      ║
║                                                                            ║
║ 5. Run the terminal again                                                 ║
╚════════════════════════════════════════════════════════════════════════════╝
                    """
                    print(oauth_guide)
                    raise Exception("Auth Code (B13) is empty - See instructions above to get it from OAuth URL")
                    
                if not Secret_Code or Secret_Code == "None" or Secret_Code.lower() == "none":
                    raise Exception("Secret Code (B12) is empty or invalid - Get from Shoonya API Dashboard (https://trade.shoonya.com/)")
                
                # Get access token using OAuth
                try:
                    result = api.getAccessToken(auth_code, Secret_Code, client_id, userid)
                    print(f"getAccessToken returned: {type(result)} = {result}")
                except Exception as oauth_err:
                    print(f"getAccessToken exception: {oauth_err}")
                    raise Exception(f"OAuth API Error: {str(oauth_err)}")
                
                if result is not None:
                    try:
                        acc_tok, usrid, ref_tok, actid = result
                        print(f"✅ Access token obtained: {acc_tok[:50]}...")
                        print(f"   Account ID: {actid}")
                        
                        # Inject OAuth header
                        print(f"🔐 Injecting OAuth headers...")
                        injected_headers = api.injectOAuthHeader(acc_tok, userid, actid)
                        print(f"✅ OAuth headers injected successfully")
                        print(f"   API is now using OAuth authentication")
                        print(f"   Headers injected: {type(injected_headers)}")
                        
                        # Test REST API call to verify OAuth is working
                        print(f"🧪 Testing REST API with OAuth token...")
                        try:
                            test_quotes = api.get_quotes(exchange='NSE', token='26000')  # NIFTY
                            if test_quotes:
                                print(f"   ✅ REST API working - Got response: {type(test_quotes)}")
                                if isinstance(test_quotes, dict) and 'lp' in test_quotes:
                                    print(f"   ✅ NIFTY LTP: {test_quotes.get('lp')}")
                            else:
                                print(f"   ⚠️  REST API returned None - May need polling fallback")
                        except Exception as test_err:
                            print(f"   ⚠️  REST API test failed: {test_err}")
                        
                        client_name = userid
                        Credential_sheet.range("c2").value = "Login Successful (OAuth), Welcome " + str(client_name) + "\nTool Validity : Demo" + "\nAccess Token Active ✓"
                        isConnected = 1
                        Text2Speech("OAuth Login Successful")
                        Credential_sheet.range('c2').color = (118,224,280)
                        
                        Message = "OAuth Login successful for userid: " + str(userid)
                        print(Message)
                        logger.info(Message)
                    except ValueError as unpack_err:
                        print(f"Token unpacking error: {unpack_err}")
                        print(f"Result value: {result}")
                        raise Exception(f"Invalid response format from OAuth: {str(unpack_err)}")
                else:
                    raise Exception("getAccessToken returned None - Check Auth Code and Secret Code validity. They may be expired or incorrect.")
                    
            except Exception as e:
                print(f"\n❌ OAuth Login Exception: {e}")
                error_msg = str(e)[:200]
                Credential_sheet.range("c2").value = f"OAuth Login Failed:\n{error_msg}"
                Text2Speech("OAuth Login unsuccessful")
...
def event_handler_quote_update(inmessage):
    ...
    try:
        ...
        if key in SYMBOLDICT:
            ...
            print(f"📊 New quote received via WebSocket: {key} = LTP: {message.get('lp', 'N/A')}")
    except Exception as e:
        print(f"❌ Error in event_handler_quote_update: {e}")
        print(f"   incoming message: {inmessage}")


def fetch_quotes_rest_api(exchange, token):
    """
    Fallback: Fetch quotes using REST API instead of WebSocket
    Used when WebSocket data is not flowing
    """
    global live_data, api
    try:
        quotes_response = api.get_quotes(exchange=exchange, token=str(token))
        
        if quotes_response:
            print(f"📡 REST API Response for {exchange}|{token}: {type(quotes_response)}")
            
            # Handle different response formats
            if isinstance(quotes_response, dict):
                if 'data' in quotes_response:
                    data = quotes_response['data']
                elif 'stat' in quotes_response and quotes_response['stat'] == 'Ok':
                ...
                live_data[key] = data
                ltp = data.get("lp") or data.get("c") or "N/A"
                print(f"   ✅ Got LTP: {ltp}")
                return data
        else:
            print(f"📡 REST API returned None for {exchange}|{token}")
            return None
    except Exception as e:
        print(f"❌ REST API Error ({exchange}|{token}): {e}")
        return None
...
def event_handler_socket_closed():
    global feed_opened
    print(f"⚠️ WebSocket closed, attempting to reconnect...")
    feed_opened = False
    sleep(2)
...
def start_Trade_Terminal():
    ...
    websocket_data_mode = None  # Track which mode we're using
    rest_api_fetch_counter = {}  # Track fetch frequency per symbol
    ...
    while True:
        try:
            ...
            # Diagnostic summary every 30 iterations
            if data_update_counter % 30 == 0:
                subscribed_count = len(subs_lst)
                websocket_data_count = len(live_data)
                
                if websocket_data_mode != ("WebSocket" if websocket_data_count > 0 else "REST API"):
                    print(f"\n📊 Data Status: Subscribed={subscribed_count}, WebSocket Data Received={websocket_data_count}")
                    if subscribed_count > 0:
                        if websocket_data_count == 0:
                            print("⚠️  WebSocket not receiving data - Using REST API polling (slower but working)")
                            websocket_data_mode = "REST API"
                        else:
                            print("✅ WebSocket is working fine")
                            websocket_data_mode = "WebSocket"
                ...
                if i:
                    ...
                    if i in subs_lst:
                        try:
                            ...
                            # Check if data exists for this token
                            if TokenKey in live_data:
                                lst = [
                                ...
                                # Fallback: Try REST API every 20 iterations if WebSocket not working
                                if TokenKey not in rest_api_fetch_counter:
                                    rest_api_fetch_counter[TokenKey] = 0
                                rest_api_fetch_counter[TokenKey] += 1
                                
                                if rest_api_fetch_counter[TokenKey] % 20 == 0:  # Fetch every ~2 seconds
                                    exchange = i[:3]
                                    token = TokenKey.split("|")[1]
                                    quote_data = fetch_quotes_rest_api(exchange, token)
                            ...
                            try:
                                ...
                                if trade_info[0] is not None and trade_info[1] is not None:
                                    if type(trade_info[0]) is float and type(trade_info[1]) is str:
                                        ...
                                        LTP = float(live_data[TokenKey].get("lp", 0)) if TokenKey in live_data else 0
...
if Shoonya_login() == 1:
    ...
    try:
        ...
        print(f"❌ WebSocket startup error: {ws_err}")
        logger.error(f"WebSocket startup failed: {ws_err}")
    ...
    while feed_opened == False:
        elapsed = time.time() - websocket_start_time
        if elapsed > websocket_timeout:
            print(f"\n❌ WebSocket connection timeout after {websocket_timeout}s")
            print("Troubleshooting:")
            print("  1. Check if your OAuth Access Token is still valid")
            print("  2. Check your Internet connection")
            print("  3. Check Shoonya server status at https://trade.shoonya.com/")
            print("  4. Try a fresh OAuth Auth Code (from the redirect URL)")
            break
        print(f"⏳ Connecting WebSocket... ({elapsed:.0f}s)")
        sleep(1)

    if feed_opened:
        print("\n✅ WebSocket connected successfully!")
    else:
        print("\n⚠️ WebSocket connection failed - Tool may have limited functionality")
